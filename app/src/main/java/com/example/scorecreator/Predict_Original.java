...
public class Predict_Original {
    ...
    public Predict_Original(int[] input) throws IOException {
        Log.d(LOG_TAG, "Prediction ");
        layer1Params = load(25, 56449, "layer1weights.txt");
        layer2Params = load(3, 26, ("layer2weights.txt"));
    ...
    private double[][] load(int layerRowSize, int layerColumnSize, String file) throws IOException {
        double[][] layerWeights = new double[layerRowSize][layerColumnSize];
        BufferedReader reader = new BufferedReader(new FileReader("R.raw." + file));
    ...
    public String predict() {
        ...
        double max = outputTwo[0][0];
        int maxPlace = 0;
        for (int i = 0; i < outputTwo[0].length; i++) {
            if (outputTwo[0][i] > max) {
                max = outputTwo[0][i];
                maxPlace = i;
            }
